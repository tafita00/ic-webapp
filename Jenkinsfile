pipeline{
    environment {
        IMAGE_NAME = "ic-webapp"
        APP_CONTAINER_PORT = "8080"
        DOCKERHUB_ID = "tafitasoa0"
        DOCKERHUB_PASSWORD = credentials('dockerhub_password')
    }
    agent none
    stages{
        stage('Build image'){
            agent any
            steps{
                script {
                    sh 'docker build --no-cache -f ./sources/app/${DOCKERFILE_NAME} -t ${DOCKERHUB_ID}/$IMAGE_NAME:$IMAGE_TAG ./sources/app'
                }
            }
        }
        stage('Scan Image with SNYK'){
            agent any
            environment{
                SNYK_TOKEN = credentials('snyk_token')
            }
            steps{
                script{
                    sh '''
                    echo "Starting Image scan $DOCKERHUB_ID/$IMAGE_NAME:$IMAGE_TAG ..."
                    echo There is Scan result:
                    SCAN_RESULT=$(docker run --rm -e SNYK_TOKEN=$SNYK_TOKEN -v /var/run/docker.sock:/var/run/docker.sock -v $(pwd):/app snyk/snyk:docker snyk test --docker $DOCKERHUB_ID/$IMAGE_NAME:$IMAGE_TAG --json || if [[$? -gt "1"]];then echo -e "Warning, you must see scan result \n"; false; elif[[$? -eq "0"]]; then echo "Pass: Nothing to Do; elif [[$? -eq "1"]]; then echo "Warning, passing with something to do"; else false; fi)
                    echo "scan ended"
                    '''
                }
            }
        }
        stage('Run container based on builded imgae'){
            agent any
            steps{
                script{
                    sh '''
                    echo "Cleanig exitsting container if exist"
                    docker ps -a | grep -i $IMAGE_NAME && docker rm -f ${IMAGE_NAME}
                    docker run --name ${IMAGE_NAME} -D -p $APP_EXPOSER_PORT:$APP_CONTAINER_PORT ${DOCKERHUB_ID}/$IMAGE_NAME:$IMAGE_TAG
                    sleep 5
                    '''
                }
            }
        }
        stage{
            agent any
            steps{
                script{
                    sh '''
                    curl -I http://${HOST_IP}:${APP_EXPOSER_PORT} | grep -i "200"
                    '''
                }
            }
        }
        stage('Clean container'){
            agent any
            steps{
                script{
                    sh '''
                    docker stop $IMAGE_NAME
                    dokerr rm $IMAGE_NAME
                    '''
                }
            }
        }
        stage('Login ant Push Image on docker hub'){
            agent any
            steps{
                script{
                    sh '''
                    echo $DOCKERHUB_PASSWORD | docker login -u $DOCKER_ID --password-stdin
                    docker push ${DOCKERHUB_ID}/$IMAGE_NAME:$IMAGE_TAG
                    '''
                }
            }
        }

    }
}
